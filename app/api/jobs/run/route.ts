...
import { getLead, saveLeadResearch, saveLeadOutreach, saveLeadProposal, saveLeadFollowup } from "@/lib/leads/store";
  ...
  async function worker() {
    while (cursor < claimed.length) {
      ...
      try {
        ...
        if (job.kind === "research") {
        ...
        } else if (job.kind === "outreach") {
          if (!leadId) throw new Error("Missing leadId");
          const lead = await getLead(leadId);
          if (!lead) throw new Error("Lead not found");
          const agent = agents.find((a) => a.id === job.agentId) ?? agents[0];
          const result = await draftOutreach(agent, lead, creatorContext);
          await saveLeadOutreach(leadId, result);
          await logActivity({
            agentId: agent?.id ?? null,
            leadId,
            type: "outreach_drafted",
            text: "Drafted an opening pitch for " + lead.name,
          });
          ...
          if (!lead) throw new Error("Lead not found");
          const agent = agents.find((a) => a.id === job.agentId) ?? agents[0];
          const result = await draftProposal(agent, lead, creatorContext, profile?.rateFloor ?? null);
          await saveLeadProposal(leadId, result);
          await logActivity({
            agentId: agent?.id ?? null,
            leadId,
            type: "proposal_drafted",
            text: "Priced a proposal for " + lead.name,
          });
          ...
          if (!lead) throw new Error("Lead not found");
          const agent = agents.find((a) => a.id === job.agentId) ?? agents[0];
          const result = await draftFollowUp(agent, lead, creatorContext);
          await saveLeadFollowup(leadId, result);
          await logActivity({
            agentId: agent?.id ?? null,
            leadId,
            type: "followup_drafted",
            text: "Drafted a follow up nudge for " + lead.name,
          });
        }
